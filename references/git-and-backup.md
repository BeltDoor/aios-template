...
How your second brain folder stays safe — and how it follows you to a new laptop.

## What this is

Your second brain folder is a **git repo**. Think of git as a time machine for the folder: every change you save gets a snapshot you can roll back to.
...
- **An encrypted off-laptop backup.** If your laptop is lost, stolen, or wiped, your second brain is still there.
...
## Two ways your second brain gets backed up

Your second brain is safe as soon as ONE of these is true, and `/day-one` handles this as its LAST setup step (after you've seen what your second brain can do), not the first:

**Option A, the easy one: your folder already lives in a cloud drive.** If your second brain sits inside OneDrive, iCloud Drive, or Dropbox, that drive already copies every change to the cloud automatically. Nothing to set up, you're backed up. `/day-one` checks for this and, if it's true, skips the GitHub step entirely.
...
2. **Pick "private."** Choose **"Publish to GitHub private repository"** and name it `secondbrain`. In one click, VS Code creates the repo under your account, connects your folder to it, and pushes the first backup.
...
Both are listed in `.gitignore`, which is git's "don't ever back this up" file. So your backup is safe to push even though your second brain folder contains real business data — the actual secrets stay local.
...
## Sharing one second brain with a teammate

Two people can share one second brain so it works as a single shared brain. The trick: one private repo, two laptops pointing at it. Do NOT have both people set up from scratch separately, or you end up with two unconnected copies.

1. **First person** sets up normally (`/day-one` publishes the private `secondbrain` repo to GitHub).
2. **Second person** does NOT clone the template. They get access to the first person's repo (either both sign into the same GitHub account, or the first person invites them as a collaborator on the repo in GitHub), then in VS Code: **Source Control → Clone Repository → pick the `secondbrain` repo**. Now both laptops point at the same private repo.
