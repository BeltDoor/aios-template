# SKILLS

The skill catalog for this second brain. One entry per skill: what it does, when to invoke it, what to listen for in my messages.

> Auto-regenerated by `/king-intelligence:end-session`. Read this before responding to my first message in any session.

Status legend: `[live]` = built and ready. `[not yet built]` = placeholder; the skill is on the roadmap but not on disk yet.

---

## The King Intelligence plugin

The full skill library came bundled with your second brain and switched on during Day One. It lives under `/king-intelligence:` commands. Type `/king-intelligence:` in chat to see them all. Two to know first: `/king-intelligence:adapt <skill>` wires a skill to my tools, and `/king-intelligence:update` pulls Jacob's latest improvements (that one needs your personal key; until you have it, you're on the free starter set).

Three skills that used to live in this folder now live there: `/king-intelligence:brainstorming`, `/king-intelligence:grill-me`, and `/king-intelligence:end-session`.

---

## `/day-one` — first-time setup walkthrough

**Status:** `[live]`

**Purpose.** Walks me through Day One of my second brain: a fast website + LinkedIn look at my business, my identity and a business snapshot written into `CLAUDE.md`, then switching on backup. Ends with a soft-ask handoff to `/skill-builder`. One-shot per client; assumes a guided session.

**Invoke when I say:**
- "set me up"
- "first time"
- "I just opened this"
- "walk me through Day One"
- or you spot a `SETUP_BUNDLE` marker in my first message (the kickoff line from members.king-intelligence.com)

---

## `/map-my-work` — find out where my work actually lives

**Status:** `[live]`

**Purpose.** Works out where my real business files are across this whole laptop — Desktop, Documents, Downloads, OneDrive, SharePoint, iCloud Drive, Dropbox — and writes it down in [`references/where-my-work-lives.md`](references/where-my-work-lives.md) so no session ever has to ask me again. Asks my permission first. Reads folder and file **names only**, never opens a document, and **never moves, copies, or renames anything**. If my folders are a mess it offers me a clean-up prompt to run later; it never tidies on its own. Runs as its own prompt during setup, alongside `/day-one`.

**Invoke when I say:**
- "map my computer"
- "where does my stuff live"
- "find my business files"
- "learn my folders"
- or when you've had to ask me twice in a session where something is

---

## `/skill-builder` — build a new skill for a recurring task

**Status:** `[live]`

**Purpose.** The compositional engine that grows new skills. Routes to `/king-intelligence:grill-me` when I have a task in mind, or `/king-intelligence:brainstorming` when I don't. Captures how long the task takes me by hand, blocks on missing connections or missing voice profile, generates the skill, tests it once with a real record, and updates this catalog.

**Invoke when I say:**
- "build me a skill"
- "automate this"
- "I keep doing X by hand"
- "I want a skill for Y"
- "what should I automate next"
- or paste a recurring task with any "turn this into a skill" framing

---

## `/begin-session` — load a topic and brief me

**Status:** `[live]`

**Purpose.** Topic-loader. Resolves a named client / project / initiative to its work folder (creating one on first use if the topic is new), fans out in parallel across the folder + auto-memory + wired connectors (Gmail, Calendar, HubSpot, Otter, Notion, Drive), and hands back a tight chat-only briefing. Empty sections get omitted entirely (no filler). Bare invocation surfaces a recent-folders menu.

**Invoke when I say:**
- "load up Atlas Brewery for me"
- "get me up to speed on Q4 Pricing"
- "where are we with <client>"
- "/begin-session" (with or without a topic)
- or I open the session by deeply engaging a specific named topic

**Do NOT invoke** for quick tasks or general questions — those don't need background context.
