---
description: Wire a King Intelligence skill to THIS client's own tools by writing per-client config to the plugin data folder. Never edits skill files.
...
!`node "${CLAUDE_PROJECT_DIR}/.claude/scripts/ki-run.mjs" adapt-context.mjs ROOT DATA 2>/dev/null || node "${CLAUDE_PLUGIN_DATA}/scripts/ki-run.mjs" adapt-context.mjs ROOT DATA 2>/dev/null || node "${CLAUDE_PLUGIN_ROOT}/scripts/ki-run.mjs" adapt-context.mjs ROOT DATA 2>&1 || echo "KI_STEP_FAILED: no copy of the runner could be found in this folder, in the toolkit data folder, or in the toolkit folder this window opened on"`
