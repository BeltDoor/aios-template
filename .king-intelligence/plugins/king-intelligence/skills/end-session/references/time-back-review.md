...
2. **The ×0.2 calibration — permanent, never skip it.** Sum the competent-peer lines, **multiply the total by 0.2 (divide by 5)**, THEN round down to the nearest 15 minutes. This was confirmed against real sessions to correct for uncalibrated estimates running 4–5x too high; this multiplier is the permanent fix, not a per-session judgment call. The logged number is always the already-calibrated one.
