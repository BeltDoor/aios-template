...
> **Companion skills.** This works best alongside `/king-intelligence:taste-visual-design` (the "make it feel expensive" layer: exact fonts, spacing, shadows, motion, and the blocks that kill the cheap AI defaults) and `/king-intelligence:taste-redesign` (audit-first mode for taking over or upgrading an existing site without breaking what works). Reach for them together when website work fires this skill. If they are not installed, apply the equivalent guidance inline from the sections below.
