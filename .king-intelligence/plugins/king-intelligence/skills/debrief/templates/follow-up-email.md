...
Read the `voiceGuidePath` from your settings for the user's full voice profile.
...
4. **Sign-off:** just the signature block, no typed name
...
<<Gmail signature block renders here — logo + the user's name + tagline>>
```
