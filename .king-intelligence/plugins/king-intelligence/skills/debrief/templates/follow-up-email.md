...
Read the `voiceGuidePath` from your settings (`## debrief` section of `references/king-intelligence-config.md`) for the user's full voice profile.
...
6. **Closer is just "Thanks," — never type the sender's name before the signature.** The Gmail signature block (logo + name + tagline) is appended to every draft. Typing a name above it duplicates the name and makes the email look amateur. End the body with a comma-closer like "Thanks," "Talk soon," or simply leave no closer if the last sentence already carries the warmth — then the signature renders. No name. No title. No phone number. Ever.
...
5. **Closer:** "Thanks," then the Gmail signature block renders the sender's name + brand. Never type the name before the signature.
...
<<Gmail signature block renders here — logo + sender name + tagline>>
```

**Why this works:**
- References something specific (renewal spreadsheet pain)
- States what the user will do next (send proposal)
- Acknowledges their timeline without being pushy
