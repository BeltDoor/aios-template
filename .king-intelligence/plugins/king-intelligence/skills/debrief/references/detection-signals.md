...
This is the breadth fix. A naive flow detects actions LAST, off the already-written summary, using crisp action-verb heuristics only. By then the summary has compressed away the soft signals, and verbs-only misses everything that wasn't an explicit "I'll do X." The result is dropped offer ideas, person-to-person connections, and meeting-prep coaching.
...
### 1. Tasks & commitments
Anything either party said they would do. The classic action-verb layer (see [detection-heuristics.md](detection-heuristics.md) for the utterance-to-intent patterns). "I'll send you X," "I'll set that up," "let me look into Y," "I owe you Z."
- Capture the owner (the user or the other person) and any deadline.

### 2. Imminent meetings + how to handle them
A future meeting was named, AND/OR someone coached the user on how to approach a person or meeting. Example: a contact spends six minutes telling the user exactly how to sell to a third party on Thursday. That is gold, and a naive skill would log "meeting John Thursday" and drop the entire playbook.
...
- Action: cross-reference into BOTH people's notes files (or knowledge graph). **Cross-contact writes require high diarization confidence** — if the line is garbled, flag it instead of writing it.
...
The other person, often a sharp operator, hands the user an idea to improve their own offer, pricing, packaging, or positioning. These are never phrased as action items, so verb-detection misses them entirely. Example: a contact redesigns the onboarding sequence ("connect the stack first, then deploy, then automate") and the pricing model ("sell 6/8/12-week plans, not hourly") in passing.
- Capture the idea verbatim-ish and what it would change.
- Usually lands in the summary + memory + possibly a decisions-log candidate or a Lane-3 "rework the offer" prompt. Don't let these evaporate.
...
- Capture the deliverable, the recurrence (one-off vs repeating, a skill candidate), and any spec details given in the room.
...
- Action: usually a Lane-2 staged invoice/payment draft (outward, money, never auto-send).
...
- Capture who owes what and by when, so the next session/debrief can chase it.

---

## Plus: relationship & working-style intel (always)

Not a "signal" to action, but capture it: retention risk, how they like to work, personal details, buying temperature, what's driving them. Feeds the summary's relationship section + memory. (Example: "I'm out of time and can't hire" is the whole reason a buyer buys.)

---

## How to run the scan

1. Read the full raw transcript once with these seven buckets in mind. Diarization may be scrambled (auto-transcription swaps speakers) — attribute by content, not just the label, and note confidence where it matters (especially for cross-contact writes, type 3).
   - **Infer dropped short answers from the other speaker's next line before flagging "unconfirmed."** Auto-transcription routinely drops one-word replies (machine, email, tool, city, budget). If the reply is missing, the answer is almost always embedded in the responder's follow-up: "okay, so for Mac, you press FN" means the speaker said Mac; "great, since you use Pipedrive" means they confirmed Pipedrive. Read the dialogue logic, not just the labeled lines. Only mark "unconfirmed"/"[not mentioned]" after that inference genuinely fails.
