...
A future meeting was named, AND/OR someone coached the user on how to approach a person or meeting. This is the "meeting coaching" pattern: a client might spend six minutes telling the user exactly how to sell to a third party, what to say, how they like to work, what to avoid. That is gold and the old skill would have logged "meeting Thursday" and dropped the entire playbook.
...
The other person, often a sharp operator, hands the user an idea to improve their own offer, pricing, packaging, or positioning. These are never phrased as action items, so verb-detection misses them entirely. For example: a client might redesign an onboarding sequence or suggest a different pricing model entirely in passing.
...
- Action: usually a Lane-2 staged invoice/Stripe draft (outward, money → never auto-send).
...
Not a "signal" to action, but capture it: retention risk, how they like to work, personal details, buying temperature, what's driving them. Feeds the summary's relationship section + memory. (Example: "I'm out of time and can't hire" is the whole reason a client buys.)
