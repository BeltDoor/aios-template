...
|---|---|
| *"We're in"* / *"Let's move forward"* / verbal yes | **Surface it for the user, don't auto-move.** Verbal yes but not yet paying → **red-zone**. Paying already → **current-client**. (No column entry should fire a job unless your own setup wires that up deliberately.) |
| First real contact, met them, no read on need yet | Create the card in **midfield** (Phase 4E default — everyone the user meets gets carded) |
| Discovery call ending with mutual fit + no existing card | Create the card in **midfield** (Phase 4E mechanics) |
| *"Not right now"* / timing pushback but real need acknowledged | Move/create the card in **probably-not** |
...
| Pattern | Likely action |
|---|---|
| Agreed price + payment terms (deposit, milestones, monthly) | Create Stripe draft invoice |
| *"Send me the invoice"* | Create AND send Stripe invoice (verify customer.email is the A/P person, NOT just primary contact — Stripe gotcha) |
...
| *"Let me post about this"* | Queue Blotato post or send to content-app pipeline |
| *"I'll write a blog post"* | Draft markdown blog post — surface for the user to review before publishing |
