...
- **HARD RULE: a reply draft ALWAYS gets the Gmail threadId — never create it standalone.** A workaround of creating a reply without the threadId "so it shows up as a separate line in the Drafts folder" is retired — it breaks correct threading. Always set `threadId` + `In-Reply-To`/`References`, and handle the visibility side with WORDS, not structure: tell the user the draft is nested at the bottom of the conversation thread (it also appears in the Drafts folder, labeled with the thread's subject). Never trade correct threading for Drafts-list visibility again.
