...
- **YES → build it INLINE.** Research, branded PDFs, a quoted fact, a price, an attachment. Inline is the default and heavy inline work is fine — real research plus attachments baked directly into the email is the model. "Needed" is **strict**: the email is wrong or incomplete without it. A mere *promise* of future work in the email ("I'll build your X next") is NOT needed — that's Lane 3.
...
- **Calendar:** if an invite already exists for the agreed next session, the action is **perfect it, not duplicate it** — rename it, swap a Google Meet link for their Zoom link, ensure a 15-30 min gap from neighbors. Prepare the corrected invite, show the exact change, and let the user trigger the guest update (perfecting a sent invite re-notifies the guest, so it's outward → their trigger).
...
**Where it lands:** save each prompt as a file in the central queue `prompts/YYYY-MM-DD-<slug>.md` AND echo it in chat. In a fresh window the user either pastes it or says "run prompts/...". Remind them to delete after use. (No per-client folder — a cross-subject prompt born from one debrief, about a different person, belongs in the central queue where all pending work is visible at a glance.)
...
Use plain words. No emojis (the user's rule, unless they say otherwise). If a lane is empty, omit its section.
