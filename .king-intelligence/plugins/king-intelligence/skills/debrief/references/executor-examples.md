...
- `Bash: gws gmail users drafts create` with `--attachment <path>` (see your own attachment-workaround notes, if any), then `gws gmail users drafts send --id <new_draft_id>`.
...
The CRM is the one configured in your settings (`crm`, `crmBoardUrl`). All reads and writes go through the CLI at the `crmApi` path from your settings (`find` / `get` / `create` / `touch` / `move` / `archive`). Full procedure: `_shared/board-card-flow.md` (if your setup ships one). Do NOT write to a retired CRM your settings say is no longer in use — historical data only.

**Move a card between columns (the only "stage change" now).**
- `node <crmApi> move <card-id> <column>`
- Gotcha: `get <card-id>` first to read its CURRENT column; passed-in/CLAUDE.md state goes stale.
- Column entries should not fire jobs unless your own setup wires that up deliberately.

**Create a new card (prospect onboarding).**
- Dedup first: `<crmApi> find "<name>"` (try person AND company name; it also covers email and archived cards).
- `<crmApi> create '{"name":"First Last","company":"...","email":"...","phone":"...","tags":["AIOS"],"notes":"title + context line","column":"midfield"}'`
- Name is the person ONLY, company is its own field. Tag when the offer fit is clear. No due dates unless your board rules say otherwise.

**Log a meeting on a card.**
- `<crmApi> touch <card-id> "<date>: met re X, summary path, next step" debrief`
- This bumps the card's last-activity, which is what drives the staleness colouring on the board.
...
- Why: The `mcp__claude_ai_Google_Calendar__*` server only exposes `__authenticate` and `__complete_authentication` — it's OAuth-only. Use the `gws calendar` CLI for actual event creation. See your own calendar-API reference, if you have one.
- Default: send invite to attendees, include video link or location from the call.
- Gotcha: TZ. Build datetimes with the user's own explicit timezone, not the machine's default.
...
**Publish a page to the user's own website**, if it's served straight from the repo (e.g. GitHub Pages).
...
- Pattern: look for existing pages under `docs/` for prior examples.
...
- `Write` to the user's auto-memory folder (see root CLAUDE.md memory section for the path) AND add a 1-line pointer to `MEMORY.md`.
