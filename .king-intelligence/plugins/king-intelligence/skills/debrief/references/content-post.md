...
3. **Render:**
   ...
   ```
   Prints the PNG path. Rendered locally via headless Chrome, using the user's own brand colors, logo, and font.

**The seam:** `render-card.mjs` is the single integration point with the branded-styles rendering. It renders templates from a local styles folder (`${CLAUDE_PLUGIN_ROOT}/skills/debrief/assets/branded-styles/` by default, overridable via `DEBRIEF_BRAND_LAB_DIR`). When that work moves to a different renderer or new styles, repoint `LAB_DIR` (or the queue) and nothing else in Phase 6 changes. Add a style = drop a `<name>.tpl.html` (standard `{{TOKENS}}`) + append the name to `state/image-queue.json` "queue". Confirm with the user where their brand assets live before assuming the default path is populated.
...
```
This builds a mock LinkedIn post (the user's avatar, name, the words, the card) and opens it in their native browser. They react:
- **"post it"** → Step 7.
...
**Primary path: the `socialPublishTool` from your settings** (Blotato MCP — the only chat-callable path that posts IMAGES to a personal LinkedIn). The user's LinkedIn account = the `socialAccountId` from your settings.
...
**Fallback path: Playwright** (reliable, handles anything Blotato chokes on, and is the path for Phase-2 carousels). Drive the user's logged-in LinkedIn via the persistent profile `~/.playwright-profile`: open the feed, start a post, attach the local card PNG directly (no public URL needed), paste the caption, publish. (Recovery if the profile is wedged: kill playwright chrome procs + rm SingletonLock.)
