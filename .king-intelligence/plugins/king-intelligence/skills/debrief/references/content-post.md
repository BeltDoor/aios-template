...
- **Gold** = a type-4 reframe, or a resonance with a specific reaction, or a clearly-articulated principle from the user. Lead with conviction.
...
- **Voice:** pass NO voice-profile path. Its default already IS the user's voice profile (the `voiceGuidePath` from your settings). That's what we want.
...
3. **Render:**
   ...
   ```
   Prints the PNG path. Black + gold, real logo, Space Grotesk — rendered locally via headless Chrome from the branded-styles lab.

**The seam:** `render-card.mjs` is the single integration point with the image work. It reads templates from a `LAB_DIR` constant in the script. When styles are added or the renderer changes, repoint `LAB_DIR` (or the render call) and nothing else in Phase 6 changes. Add a style = drop a `<name>.tpl.html` (standard `{{TOKENS}}`) + append the name to `state/image-queue.json` "queue". Check `LAB_DIR` in the script before rendering to confirm the path is current.
...
## Step 7 — Publish to the PERSONAL LinkedIn, only on "post it"

**Primary path: the `socialPublishTool` from your settings** (e.g. Blotato MCP — the chat-callable path that posts IMAGES to LinkedIn). LinkedIn account = the `socialAccountId` from your settings.
...
**Fallback path: Playwright** (reliable, handles anything Blotato chokes on, and is the path for Phase-2 carousels). Drive the logged-in LinkedIn via the persistent profile `~/.playwright-profile`: open the feed, start a post, attach the local card PNG directly (no public URL needed), paste the caption, publish. (Recovery if the profile is wedged: kill playwright chrome procs + rm SingletonLock.)

**Gate + caveats:**
- Fire ONLY on an explicit "post it." Never auto-publish (matches debrief rule 6 — nothing outward without the user's trigger).
- **No auto-comment fires on this path.** The pricing auto-comment lives only on the content app's direct-OAuth path; a Blotato/Playwright post goes out clean. That's intended for "here's what I learned" posts.
