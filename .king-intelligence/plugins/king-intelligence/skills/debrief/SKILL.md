...
- `knowledgeGraph`: the in-repo wikilinked knowledge-graph folder fed in Phase 4G. If absent, skip Phase 4G.
...
**Goal:** The user says "/debrief sam-riley" and gets a complete debrief in one pass — nothing important missed, nothing risky auto-fired, and the big work teed up as ready-to-run prompts instead of half-built in a tired window.
...
- `$ARGUMENTS` = client name (e.g., "sam-riley", "acme-co"), or `this` / a path for a pasted thread.
...
**The contract: the user NEVER runs the transcript puller themselves.** Use the `transcriptSource` from your settings (Step 0) as the source — the auto-pull recipe below is written for `otter`. If we can pull it, we pull it. Only ask the user to paste when the source genuinely has no matching meeting. If no `transcriptSource` is configured, skip the auto-pull and ask the user to paste the transcript directly.
...
4. **Zero matches: actively pull, then re-check. Do NOT bail.**
   ...
   c. Match now → load it.
   d. Still zero + the meeting was on Zoom (the user's PMR, or they say "Zoom call") → **Zoom fast path, do NOT wait for Zoom's own transcript** (its speech-to-text lags 50-85 min; the audio file lands minutes after the meeting):
...
1. **Build the roster** from what you know: **the user/host is always present**; `calendar_guests` (names + emails) and `matched_email` from the staged frontmatter Phase 0 loaded; the client's name + any named colleagues from their `CLAUDE.md`; the meeting type's expected headcount.
...
4. **Offer / strategy / business-model ideas** (the client improving your offer/pricing — never phrased as a task)
...
This is a **Lane-2** outward item AND the razor's anchor (see Phase 5). **Write the email body by invoking the `/email` skill — mandatory, every time, never hand-write it** (it enforces EMAIL-VOICE.md + the real stored signature). Build everything the email *needs* (research, facts, PDFs, a payment link for any money owed) **inline** and bake/attach it. **Always do BOTH: show the email inline in chat AND create the draft** in the `emailTool` from your settings — the user wants it sitting in drafts every time, not gated on approval. Never SEND it (the user sends). **Reconcile first** — check for an existing draft/thread before composing.
...
- **4G. Knowledge graph: compound the people + company from this meeting.** Gated on the `knowledgeGraph` from your settings: the wikilinked Obsidian graph over the repo; read its `CLAUDE.md` for the full spec. This is the LIVE half of compounding (the other half runs at `/king-intelligence:end-session`), and a meeting is the single richest source of new entities, so do it here, every debrief, the moment the intel is fresh. **Skip silently** if no `knowledgeGraph` is configured, or this was a purely internal/personal meeting that surfaced no durable new entity. Otherwise, for the person(s) met AND their company (plus any genuinely new concept/project the meeting crystallized):
  ...
  - **Create or update** the note following the graph's `CLAUDE.md` frontmatter spec: `type` (person/company/etc.), `created` (`date '+%m/%-d/%y'`), `tags`, `status` (`active`/`prospect`/`archived` for people/companies/projects), `related`, `source`. Filenames are kebab-case slugs.
  ...
  - These are in-repo files; they get committed by the normal backup, and `/king-intelligence:end-session` Phase 3 is idempotent with this step (dedup means whichever runs first creates, the other updates) so the two never fight. Report what you created/updated under "Done."
...
**The razor:** *is this needed for the follow-up email (or, no email, for what the user owes this person now)?* YES → build inline (heavy inline is fine — a research PDF baked into the email is the model; "needed" is strict, a promise of future work is NOT needed). NO → standalone → Lane 3 prompt. Genuine fence → ask once.
...
6. **Never auto-send anything outward** — email, calendar updates, invoices all stage for the user's trigger (Lane 2). The only email path in the `emailTool` from your settings is creating a draft (`drafts create` for Gmail), never `messages send` / `drafts send`.
...
9. **Verify before acting on stale context** — re-query a Trello card's current list before moving it.
10. **Ask, don't guess** — ambiguous meeting type, unclear client name, a *material* speaker who's a genuine coin flip (relabel + flag the rest yourself via /speaker-id, don't ask about every uncertain line), or a genuine inline-vs-prompt fence.
...
If your repo has a `TIME-SAVED.md` usage tracker, update your row:

- Increment "Total uses" by 1
- Recompute "Total saved (cumulative)" as `Total uses × 18 min`
