...
Before anything else, read `references/king-intelligence-config.md` from your repo root (use the Read tool on that repo-relative path; the session CWD is your repo root). Parse the `## debrief` section: it's plain `- key: value` lines (and `- crmList.activeProspect: ...` style dotted keys). Hold these settings for the whole run, every place below that says "the `<key>` from your settings" reads from here:
...
4. **Zero matches: actively pull, then re-check. Do NOT bail.**
   ...
   c. Match now → load it.
   d. Still zero + the meeting was on Zoom (your PMR, a configured Zoom link, or they say "Zoom call") → **Zoom fast path, do NOT wait for Zoom's own transcript** (its speech-to-text lags 50-85 min; the audio file lands minutes after the meeting):
...
Otter's "who said what" is routinely wrong (it splits a 2-person call into "Speaker 1-9," swaps you and the client, lets side conversations bleed in), so the transcript must be corrected, never trusted as-is.
...
1. **Every new file gets a date stamp** (`M/D/YY - HH:MM EDT`, or your local convention).
