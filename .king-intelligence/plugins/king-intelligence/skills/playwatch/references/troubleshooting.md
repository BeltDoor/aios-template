...
- **`error[validation]: --output '<tmp path>' ... is outside the current directory`** (8/2/26): `gws` now refuses to write a file outside the directory it is invoked from, so `watch.mjs`'s download into `$TMPDIR` fails whenever the script is run from the repo root. Workaround that works today: `cd "$TMPDIR" && node .../watch.mjs --url ...`. Real fix (not yet done): have `watch.mjs` set the child process cwd to the temp dir, or pass `gws` a cwd-relative output path.
- **`spawnSync gws ENOENT`**: `gws` CLI isn't installed or not on PATH. Install via the Google Workspace CLI repo + run `gws auth login` once for the account that owns the Drive file.
