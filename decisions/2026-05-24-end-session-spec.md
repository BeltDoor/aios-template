...
1. Scan `.claude/skills/` for every subfolder containing a `SKILL.md`.
...
- **Root `CLAUDE.md` stays tight.** Don't append session logs. Only touch fields explicitly defined as fillable (e.g., § 1 Identity if `/day-one` set it as fillable, refresh any `updated:` timestamp at the top). Decision 15 says ~100 lines tight; this respects that.
...
If yes: write `decisions/<YYYY-MM-DD>-<short-topic>.md` with Decision-26 `created:` timestamp at top and the synthesized decision rationale.
...
```
chore(secondbrain-session): MM/DD/YY - <one-line summary>
...
|---|---|---|
| `SKILLS.md` | Regenerated from `.claude/skills/` scan | Every run |
| `CONNECTIONS.md` | Regenerated from `claude mcp list` + PATH scan | Every run |
| `TIME-SAVED.md` | Self-ping row + total recompute | Every run |
| `second brain/<subfolder>/CLAUDE.md` | Session-log appended | If subfolder was touched this session |
| `decisions/<date>-<topic>.md` | New decision log entry | If decision detected AND client approved |
| `clients/<name>/` or `projects/<name>/` | New scaffolded folder | If org-surgery gate fired AND client approved |
| `.claude/last-session.md` | Rewritten | Every run |
| `.claude/.session-state.json` | Updated | Every run (after successful commit) |
...
- `CLAUDE.md` (root) — Decision 15 says tight, no growth
- `README.md` — preloaded template content, not user state
- `onboarding/*` — `/day-one`'s territory
- `references/*` — preloaded reference content
- `.env` — gitignored, never touched
...
1. `.claude/skills/end-session/SKILL.md` written and ≤ ~200 lines (length cap proportional to complexity — /day-one's spec was ~100; /end-session is heavier).
2. `SKILLS.md` has an `/end-session` row populated (no longer `[not yet built]`).
